...
public class KiemThuSachKeThua
{
	...
	static void suaSach() throws Exception 
	{
		...
		if (sachS != null)
		{
			...
			if (sachS instanceof SachGiaoKhoa) 
			{
				...
				int tinhTrang  = new Scanner(System.in).nextInt();
			...
			else
			{
				...
				String nhaXBan=new Scanner(System.in).nextLine();
	...
	static Sach themMotSachMoi() 
	{
		...
		try {
			...
			{ // không trùng
				...
				ngayChuoi = new Scanner(System.in).nextLine();
				ngayNhap = convertToDate(ngayChuoi);
	...
	static void xuatDSSachGK() {
		...
		//		String tong = df.format(DS.getTongSoTienSachGK());
		//		System.out.println("\tTổng số tiền: " + tong);
		//		System.out.println("\n\n\n");

	}
	static String tieuDeGiaoKhoa() {
		return String.format("|%-10s|%-15s|%-15s|%-10s|%-20s|%-30s|", "Mã sách", "Ngày nhập", "Đơn giá", "Số lượng",
				"Nhà XB", "Tình Trang");
	}
	...
	static String tieuDeChung() {
		return String.format("|%-10s|%-15s|%-15s|%-10s|%-20s|%-30s|", "Mã sách", "Ngày nhập", "Đơn giá", "Số lượng",
				"Nhà XB", "Thuế(STK) hoặc TTrang(SGK)");
	}
	static void xuatAll() {
